...
RESOLVER_VERSION = 'v40.3-source-phase-telemetry'
app = FastAPI(title='Veeb YouTube Resolver V40.3 Source Phase Telemetry', docs_url=None, redoc_url=None)
...
MP3_OUTPUT_STALL_TIMEOUT_SECONDS = max(5.0, float(os.environ.get('VEEB_MP3_OUTPUT_STALL_TIMEOUT', '20')))
PLAYBACK_START_SLA_MS = max(1000, int(os.environ.get('VEEB_PLAYBACK_START_SLA_MS', '3000')))
MP3_STARTUP_MIN_BYTES = max(4096, int(os.environ.get('VEEB_MP3_STARTUP_MIN_BYTES', '4096')))
MP3_MAX_CONCURRENT_TRANSCODES = max(1, int(os.environ.get('VEEB_MP3_MAX_CONCURRENT', '3')))
MP3_MAX_BACKGROUND_JOBS = max(0, min(
    int(os.environ.get('VEEB_MP3_MAX_BACKGROUND', str(max(0, MP3_MAX_CONCURRENT_TRANSCODES - 1)))),
    max(0, MP3_MAX_CONCURRENT_TRANSCODES - 1)))
...
_mp3_background_transcode_sem = asyncio.Semaphore(MP3_MAX_BACKGROUND_JOBS)
...
def request_purpose(request: Request | None) -> str:
    try:
        value = request.headers.get('x-veeb-purpose') if request is not None else None
    except Exception:
        value = None
    return (value or 'playback').strip().lower()


def is_background_purpose(purpose: str) -> bool:
    value = str(purpose or '').strip().lower()
    return (value in {'cache-fill', 'prefetch', 'r2-background-prefetch', 'r2-next-prefetch',
                      'r2-library-warm', 'cache-import'}
            or value.startswith('r2-background-')
...
def load_youtube_cookie_session(force: bool=False) -> bool:
    ...
    if force:
        print('v40.3 cookie file audit', json.dumps(audit), flush=True)
...
_source_background_download_sem = asyncio.Semaphore(MP3_MAX_BACKGROUND_JOBS)
_fg_anon_pool = None
...
class YtdlpEnginePool:
    ...
    async def _child(self, video_id: str, *, download: bool, timeout: float, operation: str='source', directory=None, purpose: str='playback') -> dict[str, Any]:
        background = is_background_purpose(purpose)
        if background and download and MP3_MAX_BACKGROUND_JOBS <= 0:
            raise JobError('RESOLVER_BUSY', 'Background source acquisition is disabled at the current capacity.')
        slot_wait_started = time.monotonic()
        async with source_slot(
            _extraction_sem, _source_download_sem, download=download,
            background=background, background_downloads=_source_background_download_sem
        ) as lease:
            slot_acquired = time.monotonic()
            if directory:
                try:
                    marker = Path(directory) / '.veeb-parent-timing.json'
                    temp = marker.with_suffix('.tmp')
                    temp.write_text(json.dumps({'sourceSlotWaitMs': elapsed_ms(slot_wait_started, slot_acquired)}))
                    temp.chmod(0o600)
                    temp.replace(marker)
                except OSError:
                    pass
            ...
            try:
                ...
                if not result.get('ok'):
                    raise SourceAttemptError(result.get('error') or 'Source attempt failed',
                        ...
                        startup_timing=result.get('startupTiming'))
                if directory:
    ...
    async def resolve(self, video_id: str, purpose: str) -> ResolvedMedia:
        ...
        print('v40.3 source selected', json.dumps({'videoId': video_id, 'client': self.client_name,
    ...
    async def download_source(self, video_id: str, purpose: str, *, directory=None) -> ResolvedMedia:
        result = await self._child(video_id, download=True, timeout=SOURCE_DOWNLOAD_TIMEOUT, directory=directory, purpose=purpose)
        media = self._media(video_id, result['media'], resolver_path=self.resolver_path + '-yt-dlp-download')
        media._source_evidence = result.get('evidence') or {}
        media._source_startup_timing = result.get('startupTiming') or {}
        ...
        try:
            ...
            raise
        print('v40.3 source downloaded by yt-dlp', json.dumps({'videoId': video_id, 'client': self.client_name,
    ...
    async def inspect_session(self, video_id: str) -> dict[str, Any]:
        result = await self._child(video_id, download=False, timeout=12.0, operation='session', purpose='session-diagnostic')
...
def attempt_detail(pool, error, stage='extract', suffix=''):
    return {'path': (pool.name if pool else 'cached') + suffix,
            ...
            'startupTiming': getattr(error, 'startup_timing', {})}
...
def mark_job_timing(job, name: str, *, overwrite: bool=False) -> None:
    if job is None:
        return
    timing = job.metadata.setdefault('_timing', {})
    if overwrite or name not in timing:
        timing[name] = time.monotonic()
...
def job_timing_summary(job) -> dict[str, int | None]:
    timing = job.metadata.get('_timing', {}) if job is not None else {}
    created = getattr(job, 'created', None)
    ready = getattr(job, 'ready_at', None) or timing.get('job_ready')
    return {
        'queueMs': elapsed_ms(created, timing.get('producer_start')),
        'sourceAcquireMs': elapsed_ms(timing.get('source_attempt_start'), timing.get('source_ready')),
        'encoderStartupMs': elapsed_ms(timing.get('ffmpeg_spawn_start'), timing.get('mp3_first_bytes')),
        'totalStartupMs': elapsed_ms(created, ready),
    }

def add_startup_headers(headers: dict[str, str], job) -> None:
    values = job_timing_summary(job)
    mapping = {
        'queueMs': 'X-Veeb-Startup-Queue-Ms',
        'sourceAcquireMs': 'X-Veeb-Startup-Source-Ms',
        'encoderStartupMs': 'X-Veeb-Startup-Encoder-Ms',
        'totalStartupMs': 'X-Veeb-Startup-Total-Ms',
    }
    for key, header in mapping.items():
        if values.get(key) is not None:
            headers[header] = str(values[key])
    headers['X-Veeb-Startup-SLA-Ms'] = str(PLAYBACK_START_SLA_MS)
    headers['X-Veeb-MP3-Startup-Bytes'] = str(MP3_STARTUP_MIN_BYTES)

def job_status_payload(job) -> dict[str, Any]:
    now = time.monotonic()
    media = job.metadata.get('media') if job is not None else None
    timing = job_timing_summary(job)
    error = error_detail(job.error) if getattr(job, 'error', None) else None
    if error:
        state = 'failed'
    ...
    return {
        ...
        'bytesProduced': int(job.size),
        'ageMs': elapsed_ms(job.created, now),
        'lastOutputAgeMs': elapsed_ms(job.last_byte_at, now) if job.last_byte_at else None,
        'timing': timing,
        'startupSlaMs': PLAYBACK_START_SLA_MS,
        'startupWithinSla': (timing.get('totalStartupMs') is not None and timing['totalStartupMs'] <= PLAYBACK_START_SLA_MS),
        'client': getattr(media, 'client', None),
        'sourceFormat': getattr(media, 'format_id', None),
        'resolverPath': getattr(media, 'resolver_path', None),
        'sourceTiming': job.metadata.get('sourceTiming', {}),
        'attempts': job.metadata.get('attempts', []),
        'error': error,
    }

async def prepare_live_mp3_stream(media: ResolvedMedia, video_id: str, request: Request, job=None) -> AsyncIterator[bytes]:
    ...
    cache_slot = False
    background_transcode_slot = False
    try:
        if background:
            if MP3_MAX_BACKGROUND_JOBS <= 0:
                raise JobError('RESOLVER_BUSY', 'Background transcoding is disabled at the current capacity.')
            await _mp3_background_transcode_sem.acquire()
            background_transcode_slot = True
        if cache_fill:
        ...
        if background_transcode_slot:
            _mp3_background_transcode_sem.release()
    ...
    try:
        ...
        mark_job_timing(job, 'ffmpeg_spawn_start', overwrite=True)
        ...
        mark_job_timing(job, 'ffmpeg_spawned', overwrite=True)
        if owned is not None:
        ...
        while len(startup_buffer) < MP3_STARTUP_MIN_BYTES:
            ...
            if remaining <= 0:
                raise JobError('MP3_STARTUP_TIMEOUT', f'FFmpeg did not produce {MP3_STARTUP_MIN_BYTES} MP3 startup bytes within {MP3_FIRST_BYTE_TIMEOUT_SECONDS:g} seconds.')
            try:
                ...
                raise JobError('MP3_STARTUP_TIMEOUT', f'FFmpeg did not produce {MP3_STARTUP_MIN_BYTES} MP3 startup bytes within {MP3_FIRST_BYTE_TIMEOUT_SECONDS:g} seconds.') from exc
            if not chunk:
                break
        ...
        if len(first_chunk) < MP3_STARTUP_MIN_BYTES:
            ...
            message = f'FFmpeg produced only {len(first_chunk)} startup MP3 bytes'
            if stderr:
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        print('v40.3 mp3 output stalled', json.dumps({'videoId': video_id, 'jobId': getattr(job, 'job_id', None), 'purpose': purpose, 'timeoutSeconds': MP3_OUTPUT_STALL_TIMEOUT_SECONDS, 'sourceDownloadDone': bool(owned is not None and owned.task.done()), 'ffmpegReturnCode': process.returncode}), flush=True)
                ...
                if stderr_task is not None:
                    try:
                        ...
                        pass
                if rc != 0:
                    print('v40.3 ffmpeg ended non-zero', json.dumps({'videoId': video_id, 'returnCode': rc, 'error': redact_source_error(stderr.decode('utf-8', 'replace'))}), flush=True)
                    raise JobError('FFMPEG_FAILED', 'FFmpeg failed before MP3 completion')
            except asyncio.CancelledError:
                raise
            except BaseException as exc:
                mark_job_timing(job, 'stream_failure', overwrite=True)
                detail = error_detail(exc)
                if job is not None:
                    job.metadata['terminalFailure'] = detail
                print('v40.3 mp3 stream failed', json.dumps({'videoId': video_id, 'jobId': getattr(job, 'job_id', None), 'purpose': purpose, **detail}), flush=True)
                raise
            finally:
...
async def produce_mp3(video_id: str, request: Request, job):
    ...
    if imported is not None:
        ...
        return await prepare_live_mp3_stream(imported, video_id, request, job=job)
        ...
        async with asyncio.timeout(20):
            for pool in foreground_pools():
                ...
                try:
                    ...
                    print('v40.3 source acquisition phases', json.dumps({
                    ...
                    attempts.append({'path': pool.name, 'stage': 'mp3-startup', 'ok': True,
                                     ...
                                     'elapsedMs': elapsed_ms(attempt_started, time.monotonic())})
                    ...
                    print('v40.3 source attempt failed', json.dumps({'videoId': video_id, **detail}), flush=True)
...
_media_jobs = MediaJobs(produce_mp3, max_active=MP3_MAX_CONCURRENT_TRANSCODES, max_background=MP3_MAX_BACKGROUND_JOBS)
...
async def proxy_media(request: Request, video_id: str):
    ...
    if request.method == 'HEAD':
        return Response(status_code=200, headers={'Content-Type': 'audio/mpeg', 'Accept-Ranges': 'none', 'X-Veeb-Resolver': RESOLVER_VERSION})
    job = None
    try:
        job = _media_jobs.get(video_id, request, background=background, purpose=purpose)
        ...
        headers['X-Veeb-Request-Purpose'] = purpose
        headers['X-Veeb-Job-Class'] = 'background' if background else 'foreground'
        add_startup_headers(headers, job)
        if not background and not job.metadata.get('_startup_logged'):
            job.metadata['_startup_logged'] = True
            timing = job_timing_summary(job)
            event = 'v40.3 playback startup slow' if (timing.get('totalStartupMs') or 0) > PLAYBACK_START_SLA_MS else 'v40.3 playback startup ready'
...
async def startup_session() -> None:
    ...
    get_http_client()
    init_ytdlp_pools()
    print('v40.3 ready: foreground-first MP3 jobs; preemptible background cache capacity; reserved playback lane ' + json.dumps({'maxConcurrent': MP3_MAX_CONCURRENT_TRANSCODES, 'maxBackground': MP3_MAX_BACKGROUND_JOBS, 'foregroundReserved': max(0, MP3_MAX_CONCURRENT_TRANSCODES - MP3_MAX_BACKGROUND_JOBS)}), flush=True)
...
async def health(authorization: str | None=Header(default=None)) -> dict[str, Any]:
    ...
    return {'ok': True, 'service': 'veeb-resolver', 'version': RESOLVER_VERSION,
            ...
            'maxBackgroundJobs': MP3_MAX_BACKGROUND_JOBS,
...
async def diagnose(video_id: str, authorization: str | None=Header(default=None)):
    ...
    if not codec_test['ok']:
        return JSONResponse({'ok': False, 'version': RESOLVER_VERSION, 'videoId': video_id,
    ...
    report = {'version': RESOLVER_VERSION, 'videoId': video_id, 'cookieAudit': audit,
...
@app.get('/job/{job_id}')
async def job_status(job_id: str, authorization: str | None=Header(default=None)) -> JSONResponse:
    require_auth(authorization)
    if not JOB_ID_RE.fullmatch(job_id or ''):
        return JSONResponse({'ok': False, 'code': 'INVALID_JOB_ID'}, status_code=400, headers={'Cache-Control': 'no-store'})
    job = _media_jobs.find_by_job_id(job_id)
    if job is None:
        return JSONResponse({'ok': False, 'code': 'JOB_NOT_FOUND'}, status_code=404, headers={'Cache-Control': 'no-store'})
...
async def completed_mp3(video_id: str, request: Request,
    ...
    common = {'Cache-Control': 'no-store', 'X-Veeb-Resolver': RESOLVER_VERSION}
    if job is None:
...
async def import_cached_source(video_id: str, request: Request,
    ...
    try:
        ...
        media._local_path = str(path)
        media._output_bitrate = MP3_BITRATE_KBPS
        local_request = Request({'type': 'http', 'method': 'GET', 'headers': [(b'x-veeb-purpose', b'cache-import')], '_veeb_import_media': media})
        ...
        await _media_jobs.wait(job, complete=True)
        return {'ok': True, 'version': RESOLVER_VERSION, 'mp3Complete': True,
