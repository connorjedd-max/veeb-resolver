...
app = FastAPI(title="Veeb YouTube Resolver V35", docs_url=None, redoc_url=None)
...
async def resolve_live_cold_v35(video_id: str, purpose: str) -> ResolvedMedia:
    """True cold playback: cheap direct probe and mweb+PO start together at t=0.

    The direct HTTP probe is cheap enough to race without stealing meaningful CPU.
    There is deliberately no second CPU-heavy yt-dlp racer.
    """
    init_ytdlp_pools()
    started = time.monotonic()
    direct_task = asyncio.create_task(resolve_direct_fast(video_id, purpose + "-direct"))
    pot_task = asyncio.create_task(resolve_ytdlp_foreground_v35(video_id, purpose))
    tasks = {direct_task: "innertube-direct", pot_task: "mweb-pot-foreground"}
    errors: list[str] = []
    for done in asyncio.as_completed(tasks):
        try:
            winner = await done
            print("v35 cold race won", json.dumps({
                "videoId": video_id,
                "client": winner.client,
                "resolverPath": winner.resolver_path,
                "elapsedSeconds": round(time.monotonic() - started, 3),
            ...
            errors.append(str(exc))
    raise RuntimeError("all V35 cold resolver paths failed: " + " || ".join(errors)[-2200:])
...
def build_downstream_headers(upstream: httpx.Response, media: ResolvedMedia, cache_state: str) -> dict[str, str]:
    ...
    headers["X-Veeb-Resolver"] = "foreground-first-v35"
...
async def startup_session() -> None:
    ...
    init_ytdlp_pools()
    print("v35 resolver stack warm", json.dumps({
        "elapsedSeconds": round(time.monotonic() - started, 3),
...
async def root() -> dict[str, Any]:
    return {"ok": True, "service": "veeb-resolver", "version": "v35-foreground-first"}
...
async def health(authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    return {
        ...
        "heavyPrefetch": HEAVY_PREFETCH,
        "architecture": "t0-direct-plus-single-dedicated-mweb-pot-foreground",
    }
...
async def resolve_endpoint(video_id: str, authorization: str | None = Header(default=None)) -> JSONResponse:
    ...
    return JSONResponse({
        "provider": "veeb-v35-foreground-first-resolver",
        "videoId": video_id,
        "title": media.title,
