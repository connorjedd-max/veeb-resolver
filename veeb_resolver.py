...
app = FastAPI(title="Veeb YouTube Resolver V31", docs_url=None, redoc_url=None)
...
def build_ytdlp_command(video_id: str, client_name: str) -> list[str]:
    command = [
        ...
        "--extractor-args", youtube_extractor_args(client_name),
    ]
    cookie_file = get_writable_cookie_file()
    if cookie_file:
...
async def resolve_ytdlp_client(video_id: str, purpose: str, client_name: str, resolver_path: str) -> ResolvedMedia:
    started = time.monotonic()
    async with _ytdlp_lock:
        cached = get_cached_media(video_id)
        if cached:
            return cached
        print("yt-dlp resolve attempt", json.dumps({
            "videoId": video_id,
            "purpose": purpose,
            "client": client_name,
        ...
        process = await asyncio.create_subprocess_exec(
            *build_ytdlp_command(video_id, client_name),
        ...
        try:
            ...
            process.kill()
            await process.wait()
            raise RuntimeError(f"yt-dlp {client_name} resolve timed out")
        except asyncio.CancelledError:
            process.kill()
            await process.wait()
            raise
        ...
        if process.returncode != 0:
            ...
            raise RuntimeError(f"yt-dlp {client_name} failed: " + " | ".join(tail)[-1800:])
        try:
            info = json.loads(stdout.decode("utf-8", "replace"))
        except json.JSONDecodeError as exc:
            raise RuntimeError(f"yt-dlp {client_name} returned invalid JSON: {exc}") from exc
        media_url = str(info.get("url") or "").strip()
        if not media_url.startswith(("https://", "http://")):
            raise RuntimeError(f"yt-dlp {client_name} did not return a direct URL")
        ...
        print("yt-dlp resolve success", json.dumps({
            "videoId": video_id,
            "purpose": purpose,
            "client": client_name,
            "resolverPath": resolver_path,
...
async def resolve_ytdlp_fallback(video_id: str, purpose: str) -> ResolvedMedia:
    # If valid logged-in cookies exist, give yt-dlp's own current authenticated
    # client selection one chance before invoking mweb/BotGuard. This is still
    # a fallback to our sub-second direct /player fast path, but can avoid PO
    # generation on environments where our minimal direct request is rejected.
    load_youtube_cookie_session()
    if _youtube_cookie_authenticated and YTDLP_AUTH_CLIENT != YTDLP_POT_CLIENT:
        try:
            return await resolve_ytdlp_client(
                video_id,
                purpose + "-auth",
                YTDLP_AUTH_CLIENT,
                "yt-dlp-auth-fast",
            ...
            print("yt-dlp authenticated fast fallback missed", json.dumps({
                "videoId": video_id,
                "purpose": purpose,
                "client": YTDLP_AUTH_CLIENT,
                "error": str(exc)[-1500:],
            }), flush=True)
...
def build_downstream_headers(upstream: httpx.Response, media: ResolvedMedia, cache_state: str) -> dict[str, str]:
    ...
    headers["X-Veeb-Resolver"] = "auth-innertube-v31"
...
async def root() -> dict[str, Any]:
    return {"ok": True, "service": "veeb-resolver", "version": "v31-auth-innertube"}
...
async def health(authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    return {
        ...
        "architecture": "authenticated-direct-innertube-with-auth-ytdlp-then-mweb-pot",
    }
...
async def resolve_endpoint(video_id: str, authorization: str | None = Header(default=None)) -> JSONResponse:
    ...
    return JSONResponse({
        "provider": "veeb-auth-hybrid-resolver",
        "videoId": video_id,
        "title": media.title,
