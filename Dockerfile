...
CMD ["sh", "-c", "set -e; deno eval '1+1' >/dev/null 2>&1 || true; node /opt/bgutil/server/build/main.js --port 4416 & BG_PID=$!; node /app/veeb_innertube_helper.mjs & YTJS_PID=$!; trap 'kill $BG_PID $YTJS_PID 2>/dev/null || true' EXIT INT TERM; i=0; until curl -fsS http://127.0.0.1:4416/ping >/dev/null 2>&1; do i=$((i+1)); [ $i -ge 100 ] && { echo 'POT server failed to become ready'; exit 1; }; sleep 0.2; done; echo 'POT server ready on loopback before app startup'; i=0; until curl -fsS http://127.0.0.1:4417/health >/dev/null 2>&1; do i=$((i+1)); [ $i -ge 150 ] && { echo 'YouTube.js helper not warm yet; Uvicorn will retain yt-dlp fallback'; break; }; sleep 0.2; done; exec uvicorn veeb_resolver:app --host 0.0.0.0 --port ${PORT:-10000}"]
